...
'
  ...
  quantize)
    remote_stage '
set -euo pipefail
"$VENV_PYTHON" "$REMOTE_ROOT/scripts/quantize_glm51_fp8_block128.py" --base-model-path "$BF16_OUT" --export-dir "$FP8_OUT" --cache-dir "$HF_CACHE_DIR" --devices "$QUANT_DEVICES" --trust-remote-code
'
    ;;
  stage-model)
    remote_stage '
