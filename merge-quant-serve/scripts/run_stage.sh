...
Optional:
  ...
  VLLM_EXPECTED_VERSION, VLLM_TOOL_PARSER_PATCH_COMMIT.
EOF
}
...
derive() {
  ...
  local default_vllm_extra_args='--async-scheduling --compilation-config={"cudagraph_mode":"FULL_AND_PIECEWISE"} --enable-prefix-caching --block-size=1'
  ...
  : "${VLLM_EXPECTED_VERSION:=0.19.1rc1.dev90+g5af684c31}"
  : "${VLLM_TOOL_PARSER_PATCH_PR:=https://github.com/vllm-project/vllm/pull/39253}"
  : "${VLLM_TOOL_PARSER_PATCH_REF:=refs/pull/39253/head}"
  : "${VLLM_TOOL_PARSER_PATCH_COMMIT:=920af3c7a1b29847fb237fa9a9aaedacf48e8bbd}"
  : "${ATOM_REPO_URL:=https://github.com/san-tian/ATOM.git}"
  : "${ATOM_BRANCH:=fix/mtp-arange-buffer-token-capacity}"
  : "${ATOM_PROD_COMMIT:=d5f9a49bb2b6f3e82fda35e411d3cd962c19bf15}"
  : "${PREFETCH_WORKERS:=16}"
...
PY
  ...
  export VLLM_ROCM_USE_AITER VLLM_ROCM_QUICK_REDUCE_QUANTIZATION VLLM_ROCM_USE_AITER_RMSNORM
  ...
  export VLLM_EXPECTED_VERSION VLLM_TOOL_PARSER_PATCH_PR VLLM_TOOL_PARSER_PATCH_REF
  export VLLM_TOOL_PARSER_PATCH_COMMIT
...
VLLM_TOOL_PARSER_PATCH_PR=$VLLM_TOOL_PARSER_PATCH_PR
VLLM_TOOL_PARSER_PATCH_REF=$VLLM_TOOL_PARSER_PATCH_REF
VLLM_TOOL_PARSER_PATCH_COMMIT=$VLLM_TOOL_PARSER_PATCH_COMMIT
...
python3 "$REMOTE_ROOT/scripts/stage_manifest.py" write \
  ...
  --param "vllm_rocm_use_aiter=$VLLM_ROCM_USE_AITER" \
  --param "vllm_rocm_quick_reduce_quantization=$VLLM_ROCM_QUICK_REDUCE_QUANTIZATION" \
  --param "vllm_rocm_use_aiter_rmsnorm=$VLLM_ROCM_USE_AITER_RMSNORM"
'
    ;;
  serve-backend)
    remote_stage '
