...
PY
  ...
  export GPU_MEMORY_UTILIZATION VLLM_EXTRA_ARGS FORCE_TEMPERATURE DEFAULT_MAX_TOKENS
  export NORMALIZE_TOOL_CALL_ARGUMENTS MERGE_DEVICES QUANT_DEVICES MERGE_JOBS QUANT_WORKERS
...
print_derived() {
  derive
  for name in SSH_HOST REMOTE_ROOT DATA_DISK LOCAL_SCRATCH_MOUNT SCRATCH_ROOT HF_CACHE_DIR TMPDIR PIP_CACHE_DIR OSS_WORK_DIR PEFT_ADAPTER BF16_OUT FP8_OUT LOCAL_MODEL_PATH DURABLE_MODEL_PATH MODEL_PATH ENV_FILE ATOM_SOURCE_DIR DOCKER_IMAGE TENSOR_PARALLEL_SIZE MAX_MODEL_LEN MAX_NUM_SEQS MAX_NUM_BATCHED_TOKENS GPU_MEMORY_UTILIZATION VLLM_EXTRA_ARGS MERGE_DEVICES QUANT_DEVICES MERGE_JOBS QUANT_WORKERS EXPECTED_GPU_COUNT PUBLIC_BASE_URL; do
    printf "%s=%s\n" "${name}" "${!name:-}"
  done
}
...
remote_exports() {
  derive
  local name
  for name in REMOTE_ROOT DATA_DISK LOCAL_SCRATCH_MOUNT SCRATCH_ROOT HF_CACHE_DIR SERVE_HF_CACHE_DIR TMPDIR XDG_CACHE_HOME PIP_CACHE_DIR OSS_URL OSS_SHA256 BASE_REPO OSS_WORK_DIR PEFT_ADAPTER BF16_OUT FP8_OUT LOCAL_MODEL_PATH DURABLE_MODEL_PATH MODEL_PATH ENV_FILE CONTAINER_NAME SERVED_MODEL_NAME VENV_PYTHON DOCKER_IMAGE TENSOR_PARALLEL_SIZE MAX_MODEL_LEN MAX_NUM_SEQS MAX_NUM_BATCHED_TOKENS GPU_MEMORY_UTILIZATION VLLM_EXTRA_ARGS FORCE_TEMPERATURE DEFAULT_MAX_TOKENS NORMALIZE_TOOL_CALL_ARGUMENTS MERGE_DEVICES QUANT_DEVICES MERGE_JOBS QUANT_WORKERS EXPECTED_GPU_COUNT ROCM_TORCH_VERSION ROCM_TORCH_INDEX_URL ATOM_SOURCE_DIR ATOM_REPO_URL ATOM_BRANCH ATOM_PROD_COMMIT PREFETCH_WORKERS EXTRACT_WORKERS; do
...
"$VENV_PYTHON" -m pip install safetensors huggingface_hub accelerate peft aiohttp tqdm
set +e
"$VENV_PYTHON" - <<PY
import sys
try:
...
set -euo pipefail
export HF_HOME="$HF_CACHE_DIR" HF_HUB_CACHE="$HF_CACHE_DIR/hub" TRANSFORMERS_CACHE="$HF_CACHE_DIR/transformers" PIP_CACHE_DIR
"$VENV_PYTHON" "$REMOTE_ROOT/scripts/merge_glm51_lora_sharded.py" --base-repo "$BASE_REPO" --adapter-repo "$PEFT_ADAPTER" --out "$BF16_OUT" --cache-dir "$HF_CACHE_DIR" --dtype bfloat16 --devices "$MERGE_DEVICES" --jobs "$MERGE_JOBS"
'
    ;;
  validate-bf16)
    remote_stage '
...
'
  ...
  quantize)
    remote_stage '
set -euo pipefail
"$VENV_PYTHON" "$REMOTE_ROOT/scripts/quantize_glm51_fp8_block128.py" --src "$BF16_OUT" --out "$FP8_OUT" --cache-dir "$HF_CACHE_DIR" --devices "$QUANT_DEVICES" --workers "$QUANT_WORKERS"
'
    ;;
  stage-model)
    remote_stage '
...
'
    ;;
  serve-proxy)
    remote_stage 'ATOM_ENV_FILE="$ENV_FILE" "$REMOTE_ROOT/scripts/serve_capture_proxy.sh"'
    ;;
  serve-caddy)
    remote_stage 'ATOM_ENV_FILE="$ENV_FILE" "$REMOTE_ROOT/scripts/serve_caddy_proxy.sh"'
    ;;
  smoke)
    derive
